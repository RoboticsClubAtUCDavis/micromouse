...
#include "ParentSelectorProportional.h"
#include "Evolver.h"
#include "UtilFuncs.h"
#include <algorithm>
#include <assert.h>
...
void ParentSelectorProportional::select(
    struct EvolverSettings *pSettings,
    const std::vector<Evolver::GenotypeAndFitness> &pool, size_t &parentIndex1,
    size_t &parentIndex2, std::mt19937 &generator) const {
    ...
    // Compile new list of compatible mates, and factor compatibility into the
    // fitness measure. Also keep track of most compatible if none are in the
    // speciation tolerance
    ...
    float minDisimilarity = pool[parentIndex1]._genotype->getSimilarity(
        *pool[minDisimilarityIndex]._genotype, pSettings->_excessFactor,
        pSettings->_disjointFactor, pSettings->_averageWeightDifferenceFactor,
        pSettings->_inputCountDifferenceFactor,
        pSettings->_outputCountDifferenceFactor);

    size_t numCompatibleChoose = static_cast<size_t>(
        _numCompatibleChooseRatio * static_cast<float>(poolSize));

    for (size_t i = 0; i < poolSize; i++) {
        if (i == parentIndex1)
            continue;

        float disimilarity = pool[parentIndex1]._genotype->getSimilarity(
            *pool[i]._genotype, pSettings->_excessFactor,
            pSettings->_disjointFactor,
            pSettings->_averageWeightDifferenceFactor,
            pSettings->_inputCountDifferenceFactor,
            pSettings->_outputCountDifferenceFactor);
        ...
        if (disimilarity < pSettings->_speciationTolerance) {
            ...
            fitnessSum += iAF._fitness = std::max<float>(
                0.0f, pool[i]._fitness -
                          disimilarity * pSettings->_preferSimilarFactor);
