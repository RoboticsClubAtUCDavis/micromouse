...
#include "NetworkPhenotype.h"
#include <assert.h>
#include <iostream>

using namespace neat;

NetworkPhenotype::NetworkPhenotype() : _activationMultiplier(4.0f) {
}
...
void NetworkPhenotype::create(
    size_t numInputs, size_t numOutputs,
    const std::vector<std::shared_ptr<ConnectionGene>> &connections,
    size_t numHidden) {
    ...
    for (size_t i = 0, size = connections.size(); i < size; i++) {
        ...
        if (connections[i]->_outIndex < _inputs.size() ||
            connections[i]->_outIndex >= numUnits)
            continue;
...
void NetworkPhenotype::create(const NetworkGenotype &genotype) {
    create(genotype.getNumInputs(), genotype.getNumOutputs(),
           genotype.getConnectionSet(), genotype.getNumHidden());
}
