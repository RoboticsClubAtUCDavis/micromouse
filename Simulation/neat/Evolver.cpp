...
Evolver::Evolver() : _innovationNumber(0) {
}
...
void Evolver::initialize(
    size_t numInputs, size_t numOutputs,
    std::shared_ptr<class ParentSelector> selector, std::mt19937 &generator,
    std::shared_ptr<NetworkGenotype> (*pGenotypeFactory)()) {
    clearPopulation();
    ...
    for (size_t i = 0; i < _settings._populationSize; i++) {
        ...
        newGenotype->initialize(numInputs, numOutputs, _settings._minInitWeight,
                                _settings._maxInitWeight,
                                _settings._minInitBias, _settings._maxInitBias,
                                _innovationNumber, generator);
        newGenotype->initializeAdditional(
            numInputs, numOutputs, _settings._minInitWeight,
            _settings._maxInitWeight, _settings._minInitBias,
            _settings._maxInitBias, _innovationNumber, generator);
...
void Evolver::epoch(std::mt19937 &generator) {
    ...
    std::list<size_t>
        eliteIndices; // Used to keep from deleting elites later on
    ...
    for (size_t i = _settings._numElites; i < _settings._populationSize; i++) {
        ...
        _selector->select(&_settings, _population, parentIndex1, parentIndex2,
                          generator);

        // Create offspring
        std::shared_ptr<NetworkGenotype> child = _pGenotypeFactory();

        _population[parentIndex1]._genotype->crossover(
            *_population[parentIndex2]._genotype, *child,
            _settings._disableGeneChance, _population[parentIndex1]._fitness,
            _population[parentIndex2]._fitness, generator);

        child->crossoverAdditional(
            *_population[parentIndex2]._genotype, *child,
            _settings._disableGeneChance, _population[parentIndex1]._fitness,
            _population[parentIndex2]._fitness, generator);

        if (dist01(generator) < _settings._newNodeMutationRate)
            child->mutateAddNode(_settings._minWeight, _settings._maxWeight,
                                 _settings._minBias, _settings._maxBias,
                                 _innovationNumber, generator);

        if (dist01(generator) < _settings._newConnectionMutationRate)
            child->mutateAddConnection(
                _settings._minWeight, _settings._maxWeight, _settings._minBias,
                _settings._maxBias, _innovationNumber, generator);

        child->mutatePerturbWeight(_settings._weightPerturbationChance,
                                   _settings._maxPerturbation, generator);

        child->mutateAdditional(_innovationNumber, _settings._maxPerturbation,
                                _settings._minWeight, _settings._maxWeight,
                                _settings._minBias, _settings._maxBias,
                                _innovationNumber, generator);
    ...
    for (std::list<size_t>::iterator it = eliteIndices.begin();
         it != eliteIndices.end(); it++)
        _population[*it]._genotype = nullptr;

    // Destroy old population, with the exception of the elites since they are
    // simply transfered
    clearPopulation();
