...
NetworkGenotype::ConnectionSet::ConnectionSet(const ConnectionSet &other) {
    ...
    for (size_t i = 0; i < numOtherConnections; i++)
        _connections.push_back(std::shared_ptr<ConnectionGene>(
            new ConnectionGene(*other._connections[i])));

    for (size_t i = 0; i < numOtherNodes; i++)
        _nodes.push_back(other._nodes[i]);
}

NetworkGenotype::ConnectionSet &NetworkGenotype::ConnectionSet::
operator=(const NetworkGenotype::ConnectionSet &other) {
    removeConnections();
...
void NetworkGenotype::ConnectionSet::addConnection(
    float minBias, float maxBias,
    const std::shared_ptr<ConnectionGene> &connection,
    InnovationNumberType &innovationNumber, std::mt19937 &generator) {
    ...
    // Automatically add nodes as needed. Not initialized until it has
    // connections
    if (connection->_outIndex >= getNumNodes())
        setNumNodes(connection->_outIndex + 1);

    // Node initialization: Initialize biases and innovation numbers if not
    // already done so (no prior connections)
...
void NetworkGenotype::ConnectionSet::addConnectionKnownBias(
    float bias1, float bias2, const std::shared_ptr<ConnectionGene> &connection,
    InnovationNumberType innovationNumber1,
    InnovationNumberType innovationNumber2) {
    _connections.push_back(connection);
...
void NetworkGenotype::removeUnusedNodes() {
    ...
    for (size_t i = _numInputs; i < lastHiddenNodeIndex;) {
        ...
        if (_connectionSet._nodes[i]._connections.size() >= 2) {
            ...
            std::list<std::shared_ptr<ConnectionGene>> &connections =
                _connectionSet._nodes[i]._connections;

            for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                     connections.begin();
            ...
            if (!hasInput || !hasOutput) {
                // Remove any connections to the node TODO: NEED TO REMOVE
                // REFERENCES OTHER NODES HAVE TO IT AS WELL!
                /*for(size_t j = 0; j < _connectionSet._connections.size();)
                {
                if(_connectionSet._connections[j]->_inIndex == i ||
                _connectionSet._connections[j]->_outIndex == i)
                {
                delete _connectionSet._connections[j];

                _connectionSet._connections.erase(_connectionSet._connections.begin()
                + j);
...
bool NetworkGenotype::ConnectionSet::canSeverWithoutOrhpan(
    const ConnectionGene &connection) const {
...

    {
        ...
        const std::list<std::shared_ptr<ConnectionGene>> &inputNodeConnections =
            _nodes[connection._inIndex]._connections;

        for (std::list<std::shared_ptr<ConnectionGene>>::const_iterator it =
                 inputNodeConnections.begin();
             it != inputNodeConnections.end(); it++) {
...

    {
        ...
        const std::list<std::shared_ptr<ConnectionGene>> &inputNodeConnections =
            _nodes[connection._outIndex]._connections;
...
void NetworkGenotype::initialize(size_t numInputs, size_t numOutputs,
                                 float minWeight, float maxWeight,
                                 float minBias, float maxBias,
                                 InnovationNumberType &innovationNumber,
    ...
    for (size_t i = 0; i < _numInputs; i++)
        for (size_t j = 0; j < _numOutputs; j++) {
            ...
            connection->_outIndex =
                _numInputs + j; // Indices of the outputs start at _numInputs
            ...
            _connectionSet.addConnection(minBias, maxBias, connection,
                                         innovationNumber, generator);
        }
}
...
void NetworkGenotype::updateNumHiddenNeurons() {
    ...
    // Get total number of neurons by parsing connections and counting unique
    // connection index occurences
    // std::unordered_set<size_t> indexOccurences;
    size_t largestIndex = 0;

    for (size_t i = 0, size = _connectionSet._connections.size(); i < size;
         i++) {
...
void NetworkGenotype::mutatePerturbWeight(float perturbationChance,
                                          float maxPerturbation,
                                          std::mt19937 &generator) {
    assert(_numInputs > 0); // Check to make sure that the genotype was created

    std::uniform_real_distribution<float> dist01(0.0f, 1.0f);
    std::uniform_real_distribution<float> perturbationDist(-maxPerturbation,
                                                           maxPerturbation);

    for (size_t i = 0, size = _connectionSet._connections.size(); i < size; i++)
        if (dist01(generator) < perturbationChance)
            _connectionSet._connections[i]->_weight +=
                perturbationDist(generator);
...
void NetworkGenotype::mutatePerturbWeightClamped(
    float perturbationChance, float maxPerturbation, float minWeight,
    float maxWeight, float minBias, float maxBias, std::mt19937 &generator) {
    assert(_numInputs > 0); // Check to make sure that the genotype was created
    ...
    for (size_t i = 0, size = _connectionSet._connections.size(); i < size; i++)
        if (dist01(generator) < perturbationChance)
            _connectionSet._connections[i]->_weight = std::min(
                maxWeight,
                std::max(minWeight, _connectionSet._connections[i]->_weight));

    // Biases
    for (size_t i = 0, size = _connectionSet._nodes.size(); i < size; i++)
        if (dist01(generator) < perturbationChance)
            _connectionSet._nodes[i]._bias = std::min(
                maxBias, std::max(minBias, _connectionSet._nodes[i]._bias));
}

bool NetworkGenotype::mutateAddConnection(
    float minWeight, float maxWeight, float minBias, float maxBias,
    InnovationNumberType &innovationNumber, std::mt19937 &generator) {
    assert(_numInputs > 0); // Check to make sure that the genotype was created
    ...
    std::uniform_int_distribution<int> distRandNodeIndex1(
        _numInputs, _connectionSet.getNumNodes() - 1);

    int randNodeIndex1 = distRandNodeIndex1(generator);

    // If it is connect to every possible node, get a different node
    if (_connectionSet._nodes[randNodeIndex1]._connections.size() >=
        _connectionSet.getNumNodes()) {
        bool found = false;

        for (size_t offset = 1; offset < numNonInputNodes; offset++) {
            size_t checkIndex =
                wrap(randNodeIndex1 + offset, _connectionSet.getNumNodes());
            ...
            if (_connectionSet._nodes[checkIndex]._connections.size() <
                _connectionSet.getNumNodes()) {
    ...
    std::uniform_int_distribution<int> distRandNodeIndex2(
        0, _connectionSet.getNumNodes() - 1);

    size_t randNodeIndex2 = distRandNodeIndex2(generator);

    // If connected to self and self connections are not enabled, get new index
    // (increment and wrap)
    ...
    std::list<std::shared_ptr<ConnectionGene>> &node1List =
        _connectionSet._nodes[randNodeIndex1]._connections;
    ...
    for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
        ...
        if ((*it)->_inIndex == randNodeIndex2) {
            ...
            // Have to make sure that no attempt is being made to connect to
            // self again
            if (!_allowSelfConnections &&
                randNodeIndex1 == randNodeIndex2 + 1) {
                // Cannot connect to self, increment index
                randNodeIndex2 =
                    wrap(randNodeIndex2 + 2, _connectionSet.getNumNodes());
            } else
                randNodeIndex2 =
                    wrap(randNodeIndex2 + 1, _connectionSet.getNumNodes());
    ...
    _connectionSet.addConnection(minBias, maxBias, connection, innovationNumber,
                                 generator);

    return true;
}

void NetworkGenotype::mutateAddNode(float minWeight, float maxWeight,
                                    float minBias, float maxBias,
                                    InnovationNumberType &innovationNumber,
                                    std::mt19937 &generator) {
    assert(_numInputs > 0); // Check to make sure that the genotype was created
    ...
    const size_t newNodeIndex =
        _connectionSet.getNumNodes(); // New node will be at back of vector

    // Take random connection, split it, and insert the new node
    assert(_connectionSet.getNumConnections() > 0);

    std::uniform_int_distribution<int> distConnection(
        0, _connectionSet.getNumConnections() - 1);

    int randConnectionIndex = distConnection(generator);

    std::shared_ptr<ConnectionGene> geneToBeSplit =
        _connectionSet._connections[randConnectionIndex];
    ...
    _connectionSet.addConnection(minBias, maxBias, connection1,
                                 innovationNumber, generator);
    ...
    _connectionSet.addConnection(minBias, maxBias, connection2,
                                 innovationNumber, generator);
}

void NetworkGenotype::crossover(const NetworkGenotype &otherParent,
                                NetworkGenotype &child, float disableGeneChance,
                                float fitnessForThis,
                                float fitnessForOtherParent,
                                std::mt19937 &generator) {
#ifdef DEBUG
    for (size_t i = 0, numConnections = _connectionSet.GetNumConnections();
         i < numConnections; i++)
        assert(_connectionSet._connections[i]->_outIndex >= _numInputs);

    for (size_t i = 0,
                numConnections = otherParent._connectionSet.GetNumConnections();
         i < numConnections; i++)
        assert(otherParent._connectionSet._connections[i]->_outIndex >=
               otherParent._numInputs);
#endif
    ...
    std::list<std::shared_ptr<ConnectionGene>>
        notMatchingHistoryConnectionGenes1;
    ...
    for (size_t i = 0; i < otherParent._connectionSet.getNumConnections(); i++)
        otherParentInnovationNumbers.push_back(
            otherParent._connectionSet._connections[i]);

    for (size_t i = 0; i < _connectionSet.getNumConnections(); i++) {
        bool matchFound = false;

        for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                 otherParentInnovationNumbers.begin();
             it != otherParentInnovationNumbers.end();) {
            if (_connectionSet._connections[i]->_innovationNumber ==
                (*it)->_innovationNumber) {
        ...
        if (!matchFound)
            notMatchingHistoryConnectionGenes1.push_back(
                _connectionSet._connections[i]);
    }
    ...
    for (std::list<GenePair>::iterator it =
             matchingHistoryConnectionGenes.begin();
         it != matchingHistoryConnectionGenes.end(); it++) {
        ...
        child._connectionSet.addConnectionKnownBias(
            pSelected->_connectionSet._nodes[connection->_inIndex]._bias,
            pSelected->_connectionSet._nodes[connection->_outIndex]._bias,
            connection,
            pSelected->_connectionSet._nodes[connection->_inIndex]
                ._innovationNumber,
            pSelected->_connectionSet._nodes[connection->_outIndex]
                ._innovationNumber);

        // Random disable/enable
        if ((!it->_gene1->_enabled || !it->_gene2->_enabled) &&
            dist01(generator) < disableGeneChance)
            connection->_enabled = false;
        else
            connection->_enabled = true;
    }

    // notMatchingHistoryConnectionGenes1 and otherParentInnovationNumbers hold
    // disjoint/excess genes from this or other parent respectively now Give
    // child the disjoint genes from the more fit parent If they have the same
    // fitness, take some randomly from both parents
    enum SelectionMode { _this, _other, _both } selectionMode = _both;

    const size_t minNumConnections =
        std::min<size_t>(_connectionSet.getNumConnections(),
                         otherParent._connectionSet.getNumConnections());

    if (fitnessForThis > fitnessForOtherParent) {
        selectionMode = _this;

        for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                 notMatchingHistoryConnectionGenes1.begin();
             it != notMatchingHistoryConnectionGenes1.end(); it++) {
            if ((*it)->_outIndex < child._numInputs)
                continue;

            std::shared_ptr<ConnectionGene> connection(
                new ConnectionGene(*(*it)));

            child._connectionSet.addConnectionKnownBias(
                _connectionSet._nodes[connection->_inIndex]._bias,
        ...
        for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                 otherParentInnovationNumbers.begin();
             it != otherParentInnovationNumbers.end(); it++) {
            if ((*it)->_outIndex < child._numInputs)
                continue;
            ...
            child._connectionSet.addConnectionKnownBias(
                otherParent._connectionSet._nodes[connection->_inIndex]._bias,
                otherParent._connectionSet._nodes[connection->_outIndex]._bias,
                connection,
                otherParent._connectionSet._nodes[connection->_inIndex]
                    ._innovationNumber,
                otherParent._connectionSet._nodes[connection->_inIndex]
                    ._innovationNumber);
    ...
    } else if (!notMatchingHistoryConnectionGenes1.empty() ||
               !otherParentInnovationNumbers.empty()) {
        selectionMode = _both;

        // Number of disjoint/excess genes inherited is average of number of
        // genes from each
        size_t numInherit;

        if (minNumConnections > child._connectionSet.getNumConnections())
            numInherit = std::max<size_t>(
                minNumConnections - child._connectionSet.getNumConnections(),
                static_cast<size_t>(
                    static_cast<float>(
                        notMatchingHistoryConnectionGenes1.size() +
                        otherParentInnovationNumbers.size()) *
                    0.5f));
        else
            numInherit = static_cast<size_t>(
                static_cast<float>(notMatchingHistoryConnectionGenes1.size() +
                                   otherParentInnovationNumbers.size()) *
                0.5f);
        ...
        for (size_t i = 0; i < numInherit; i++) {
            ...
            if ((dist01(generator) < 0.5f &&
                 !notMatchingHistoryConnectionGenes1.empty()) ||
                otherParentInnovationNumbers.empty()) {
                std::uniform_int_distribution<int>
                    distNotMatchingHistoryConnectionGenes1(
                        0, notMatchingHistoryConnectionGenes1.size() - 1);

                int randIndex =
                    distNotMatchingHistoryConnectionGenes1(generator);

                // Get pointer by advancing iterator
                std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                    notMatchingHistoryConnectionGenes1.begin();
                ...
                child._connectionSet.addConnectionKnownBias(
                    ...
                    connection,
                    _connectionSet._nodes[connection->_inIndex]
                        ._innovationNumber,
                    _connectionSet._nodes[connection->_inIndex]
                        ._innovationNumber);

#ifdef DISABLE_CHANCE_EXCESS_DISJOINT
                if (!connection->_enabled &&
                    dist01(generator) < disableGeneChance)
                    connection->_enabled = false;
                else
                    connection->_enabled = true;
                ...
                std::uniform_int_distribution<int>
                    distOtherParentInnovationNumbers(
                        0, otherParentInnovationNumbers.size() - 1);
                ...
                child._connectionSet.addConnectionKnownBias(
                    otherParent._connectionSet._nodes[connection->_inIndex]
                        ._bias,
                    otherParent._connectionSet._nodes[connection->_outIndex]
                        ._bias,
                    connection,
                    otherParent._connectionSet._nodes[connection->_inIndex]
                        ._innovationNumber,
    ...
    // If not enough, inherit from other one after all in addition to these
    // connections
    if (child._connectionSet._connections.size() < minNumConnections) {
        ...
        if (selectionMode == _this) {
            ...
            for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                     otherParentInnovationNumbers.begin();
                 it != otherParentInnovationNumbers.end() &&
                 child._connectionSet._connections.size() < minNumConnections;
                 it++) {
                if ((*it)->_outIndex < child._numInputs)
                    continue;
            ...
            for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                     notMatchingHistoryConnectionGenes1.begin();
                 it != notMatchingHistoryConnectionGenes1.end() &&
                 child._connectionSet._connections.size() < minNumConnections;
                 it++) {
                if ((*it)->_outIndex < child._numInputs)
                    continue;
                ...
                child._connectionSet.addConnectionKnownBias(
                    ...
                    _connectionSet._nodes[connection->_inIndex]
                        ._innovationNumber,
                    _connectionSet._nodes[connection->_outIndex]
                        ._innovationNumber);
    ...
    for (size_t i = 0,
                numConnections = child._connectionSet._connections.size();
         i < numConnections; i++) {
        if (child._connectionSet._connections[i]->_enabled) {
            // Re-enable if it cannot be severed
            if (!child._connectionSet.canSeverWithoutOrhpan(
                    *child._connectionSet._connections[i]))
...
#ifdef DEBUG
    for (size_t i = 0,
                numConnections = child._connectionSet.GetNumConnections();
         i < numConnections; i++)
        assert(child._connectionSet._connections[i]->_outIndex >=
               child._numInputs);
#endif
}

float NetworkGenotype::getSimilarity(const NetworkGenotype &other,
                                     float excessFactor, float disjointFactor,
    ...
    for (size_t i = 0, numConnections_otherParent =
                           other._connectionSet.getNumConnections();
         i < numConnections_otherParent; i++) {
        if (other._connectionSet._connections[i]->_innovationNumber >
            maxInnovation2)
            maxInnovation2 =
                other._connectionSet._connections[i]->_innovationNumber;

        otherGeneInnovationNumbers.push_back(
            other._connectionSet._connections[i]);
    }

    // Count max innovation number for first gene
    InnovationNumberType maxInnovation1 = 0;

    for (size_t i = 0, numConnections_this = _connectionSet.getNumConnections();
         i < numConnections_this; i++)
    ...
    for (size_t i = 0, numConnections_this = _connectionSet.getNumConnections();
        ...
        std::list<std::shared_ptr<ConnectionGene>>::iterator it =
            otherGeneInnovationNumbers.begin();
        ...
        if (matchFound) {
            ...
            totalWeightDifference += std::abs(
                (*it)->_weight - _connectionSet._connections[i]->_weight);
    ...
    // Now add in the biases for the matching genes (node innovation number
    // matching)

    // Build list of unmatched indices so don't repeat search
    std::list<size_t> otherNodeUnmatchedIndices;

    for (size_t i = 0, numNodes2 = other._connectionSet.getNumNodes();
         i < numNodes2; i++)
        otherNodeUnmatchedIndices.push_back(i);

    for (size_t i = 0, numNodes1 = _connectionSet.getNumNodes(); i < numNodes1;
         i++)
        for (std::list<size_t>::iterator it = otherNodeUnmatchedIndices.begin();
             it != otherNodeUnmatchedIndices.end(); it++) {
            if (_connectionSet._nodes[i]._innovationNumber ==
                other._connectionSet._nodes[*it]._innovationNumber) {
                totalWeightDifference +=
                    fabsf(_connectionSet._nodes[i]._bias -
                          other._connectionSet._nodes[*it]._bias);
    ...
    // Now count other's excess/disjoint from remains in
    // otherGeneInnovationNumbers
    for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
             otherGeneInnovationNumbers.begin();
         it != otherGeneInnovationNumbers.end(); it++) {
    ...
    float normalizationFactor =
        1.0f / std::max<size_t>(_connectionSet.getNumConnections(),
                                other._connectionSet.getNumConnections());
#else
    ...
    if (numMatching == 0) // Avoid / 0
        return getSimilarityAdditional(other, excessFactor, disjointFactor,
                                       averageWeightDifferenceFactor,
                                       inputCountDifferenceFactor,
                                       outputCountDifferenceFactor) +
               excessFactor *
                   static_cast<float>(numExcessConnectionGenes1 +
                                      numExcessConnectionGenes2) *
                   normalizationFactor +
               disjointFactor *
                   static_cast<float>(numDisjointConnectionGenes1 +
                                      numDisjointConnectionGenes2) *
                   normalizationFactor +
               inputCountDifferenceFactor *
                   std::abs(static_cast<float>(_numInputs) -
                            static_cast<float>(other._numInputs)) +
               outputCountDifferenceFactor *
                   std::abs(static_cast<float>(_numOutputs) -
                            static_cast<float>(other._numOutputs));
    ...
    return getSimilarityAdditional(other, excessFactor, disjointFactor,
           ...
           disjointFactor *
               ...
               normalizationFactor +
           averageWeightDifferenceFactor *
               (totalWeightDifference / numMatching) +
           inputCountDifferenceFactor *
               std::abs(static_cast<float>(_numInputs) -
                        static_cast<float>(other._numInputs)) +
           outputCountDifferenceFactor *
...
void NetworkGenotype::setNumInputs(size_t numInputs) {
    ...
    if (dNodes > 0) {
        ...
        _connectionSet._nodes.insert(_connectionSet._nodes.begin() + _numInputs,
                                     dNodes, NodeData());
    } else if (dNodes < 0) {
        for (size_t i = 0; i < _connectionSet.getNumConnections();) {
            // If it was connected to an input that is now removed, delete the
            // connection
            if (_connectionSet._connections[i]->_inIndex < _numInputs &&
                _connectionSet._connections[i]->_inIndex >= numInputs) {
                // Remove references from nodes it connects
                NodeData &node =
                    _connectionSet
                        ._nodes[_connectionSet._connections[i]->_inIndex];

                bool found = false;

                for (std::list<std::shared_ptr<ConnectionGene>>::iterator it =
                         node._connections.begin();
                     it != node._connections.end(); it++)
                ...
                node = _connectionSet
                           ._nodes[_connectionSet._connections[i]->_outIndex];
                ...
                _connectionSet._connections.erase(
                    _connectionSet._connections.begin() + i);
            } else {
                if (_connectionSet._connections[i]->_inIndex >= _numInputs)
                    _connectionSet._connections[i]->_inIndex += dNodes;

                // assert(_connectionSet._connections[i]->_outIndex >=
                // _numInputs);
        ...
        _connectionSet._nodes.erase(_connectionSet._nodes.begin() + numInputs,
                                    _connectionSet._nodes.begin() + _numInputs);
    }
...
void NetworkGenotype::setNumOutputs(size_t numOutputs) {
    ...
    if (dNodes < 0) {
        for (size_t i = 0; i < _connectionSet.getNumConnections();) {
            // If it was connected to an input that is now removed, delete the
            // connection
            if (_connectionSet._connections[i]->_inIndex >= numOutputs ||
                _connectionSet._connections[i]->_outIndex >= numOutputs) {
                // Remove references from nodes it connects
                NodeData &node =
                    _connectionSet
                        ._nodes[_connectionSet._connections[i]->_inIndex];
...
void NetworkGenotype::setNumInputsFullyConnect(
    size_t numInputs, float minWeight, float maxWeight, float minBias,
    float maxBias, InnovationNumberType &innovationNumber,
    std::mt19937 &generator) {
#ifdef DEBUG
    for (size_t i = 0, numConnections = _connectionSet.GetNumConnections();
         i < numConnections; i++)
    ...
    const int newNumNodes =
        static_cast<int>(numInputs + _numHidden + _numOutputs);
    ...
    if (dNodes > 0) {
        ...
        for (int i = 0; i < dNodes; i++)
            for (size_t j = 0; j < _numOutputs; j++) {
                ...
                assert(newNumNodes - 1 - static_cast<signed>(j) >=
                       static_cast<signed>(numInputs));
...
void NetworkGenotype::setNumOutputsFullyConnect(
    size_t numOutputs, float minWeight, float maxWeight, float minBias,
    float maxBias, InnovationNumberType &innovationNumber,
    std::mt19937 &generator) {
#ifdef DEBUG
    for (size_t i = 0, numConnections = _connectionSet.GetNumConnections();
    ...
    if (dNodes > 0) {
        ...
        for (int i = 0; i < dNodes; i++)
            for (size_t j = 0; j < _numInputs; j++) {
                ...
                assert(static_cast<signed>(newNumNodes) - 1 -
                           static_cast<signed>(i) >
                       static_cast<signed>(_numInputs));
...
void NetworkGenotype::connectUnconnectedInputs(
    float minWeight, float maxWeight, float minBias, float maxBias,
    InnovationNumberType &innovationNumber, std::mt19937 &generator) {
    // ---------------------------------- Add "Missing" Connections for Inputs
    // and Outputs ----------------------------------
...
void NetworkGenotype::connectUnconnectedOutputs(
    float minWeight, float maxWeight, float minBias, float maxBias,
    InnovationNumberType &innovationNumber, std::mt19937 &generator) {
    // ---------------------------------- Add "Missing" Connections for Inputs
    // and Outputs ----------------------------------
...
void NetworkGenotype::readFromStream(std::istream &is) {
    ...
    for (size_t i = 0; i < numConnections; i++) {
        ...
        is >> connection->_enabled >> connection->_inIndex >>
            connection->_outIndex >> connection->_weight >>
            connection->_innovationNumber;
        ...
        _connectionSet._nodes[connection->_inIndex]._connections.push_back(
            connection);
        _connectionSet._nodes[connection->_outIndex]._connections.push_back(
            connection);
    }
    ...
    for (size_t i = 0; i < numNodes; i++)
        is >> _connectionSet._nodes[i]._bias >>
            _connectionSet._nodes[i]._innovationNumber;
}
...
void NetworkGenotype::writeToStream(std::ostream &os) const {
    ...
    os << _connectionSet.getNumConnections() << " "
       << _connectionSet.getNumNodes() << std::endl;
    ...
    for (size_t i = 0, numConnections = _connectionSet.getNumConnections();
        ...
        std::shared_ptr<ConnectionGene> connection =
            _connectionSet._connections[i];
        os << connection->_enabled << " " << connection->_inIndex << " "
           << connection->_outIndex << " " << connection->_weight << " "
           << connection->_innovationNumber << std::endl;
    }

    // Write all nodes (biases and innovation numbers)
    for (size_t i = 0, numNodes = _connectionSet.getNumNodes(); i < numNodes;
         i++) {
